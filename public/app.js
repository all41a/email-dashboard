...
function App() {
  ...
  const loadMeta = useCallback(async () => {
    const [s, c, a, v, p, d] = await Promise.all([
  ...
  const loadEmails = useCallback(async () => {
    ...
    params.set('view', isCat ? 'inbox' : view);
  ...
  // ---------------------------------------------------------- render
  const viewTitle = view.startsWith('cat:') ? view.slice(4) : (VIEWS[view]?.label || 'Inbox');
...
// --------------------------------------------------------------- Sidebar
function Sidebar({ open, view, setView, stats, categories, presets, drafts, dragOverCat, setDragOverCat, onDropCategory, applyPreset, deletePreset }) {
  const counts = {
...
// ------------------------------------------------------------ EmailDetail
function EmailDetail({ email: e, catMap, vipEmails, onClose, onStar, onArchive, onDelete, onSpam, onUnsubscribe, onVip, onRemoveVip, onReply, onOpenRelated, onCategorize, categories, toast }) {
